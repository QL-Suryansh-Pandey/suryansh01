...
test('new user can sign up', async ({ page }) => {
  ...
  await test.step('Open signup page', () => signupPage.open());
  await test.step('Fill and submit workspace signup form', () => signupPage.signup(user));
  await test.step('Assert redirected to OTP verification', () =>
    expect(page).toHaveURL(/otp-verification/i, { timeout: 15000 }));
});
...
test('weak password shows a validation error and stays on signup', async ({ page }) => {
  ...
  await test.step('Open signup page', () => signupPage.open());
  await test.step('Submit form with a weak password', () => signupPage.signup(user));
